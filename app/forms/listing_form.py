...
class ListingForm(FlaskForm):
    ...
    city = StringField('city', validators=[DataRequired('Please provide a city.'), Length(min=2, max=24, message='City must be between 2-24 characters')])
    state = StringField('state', validators=[DataRequired('Please provide a state.'), Length(min=2, max=24, message='State must be between 2-24 characters')])
