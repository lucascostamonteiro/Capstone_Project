...
class ListingForm(FlaskForm):
    ...
    address = StringField('address', validators=[DataRequired('Please provide an address.'), Length(min=5, max=25, message='Address must be between 5-25 characters')])
    city = StringField('city', validators=[DataRequired('Please provide a city.'), Length(min=3, max=25, message='City must be between 3-25 characters')])
